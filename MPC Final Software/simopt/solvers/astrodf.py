...
class ASTRODF(Solver):
    ...
    def construct_model(self, x_k, delta_k, k, problem, expended_budget, kappa, new_solution, visited_pts_list):
        interpolation_solns = []
        ...
        delta_k = min(max(beta * norm(grad), delta_k), delta_k)
    ...
    def solve(self, problem):
        ...
        if not (np.isinf(problem.upper_bounds).any() or np.isinf(problem.lower_bounds).any()):
            ...
            delta_k = delta_coefficient * 0.1 * min(np.array(problem.upper_bounds) - np.array(problem.lower_bounds))
        else:
            delta_k = delta_coefficient * 10 ** (ceil(log(delta_max * 2, 10) - 1) / problem.dim)
