...
Summary
-------
ASTRO-BFDF for bi-fidelity simulation optimization problems for EXAMPLE problem.
...
class EXASTROBFDF(Solver):
    """The ASTRO-BFDF solver for EXAMPLE problem.
    ...
    base.Solver
    """
    def __init__(self, name="EXASTROBFDF", fixed_factors=None):
        ...
        self.specifications = {
            ...
            "delta_max": {
                "description": "maximum trust region size",
                "datatype": float,
                "default": 100
            },
            "delta": {
                "description": "the initial trust region sizes",
                "datatype": list,
                "default": [1, 1]
            },
            "batch_size": {
                "description": "the number of additional simulation replications added per adaptive-sampling update",
                "datatype": list,
                "default": [1, 10]
            },
        }
    ...
    def mfmc_sample_size(self, new_solution, o1_solution_set, problem, budget_limit, expended_budget, kappa, k, delta):
        ...
        batch_sizes = self.factors["batch_size"]
        ...
        # pilot run for high-fidelity if needed
        if pilot_run - new_solution.n_reps > 0:
            problem.model.factors["target_fidelity"] = "HF"
            batch = pilot_run - new_solution.n_reps
            problem.simulate(new_solution, batch)
            expended_budget += batch * costs[0]
        
        # pilot run for low-fidelity if needed
        if pilot_run - o1_solution_set[o1_id].n_reps > 0:
            problem.model.factors["target_fidelity"] = "LF"
            problem.simulate(o1_solution_set[o1_id], pilot_run - o1_solution_set[o1_id].n_reps)
            expended_budget += costs[1]*(pilot_run - o1_solution_set[o1_id].n_reps)
        ...
        while True:
            ...
            if solve_subproblem_approximate.fun <= costs[0]*predicted_N_k:
                ...
                if new_solution.n_reps > o1_solution_set[o1_id].n_reps:        
                    problem.model.factors["target_fidelity"] = "LF"
                    batch_lf = new_solution.n_reps - o1_solution_set[o1_id].n_reps + 1
                    problem.simulate(o1_solution_set[o1_id], batch_lf)
                    expended_budget += batch_lf*costs[1]

                # check whetehr we can finish the sampling
                lf_mean_n = np.mean(o1_solution_set[o1_id].lf_f[:new_solution.n_reps])
                fn = new_solution.objectives_mean - c1*lf_mean_n  + c1*o1_solution_set[o1_id].lf_f_mean 
                ...
                if new_solution.n_reps >= ceil(n0)-1:
                    problem.model.factors["target_fidelity"] = "LF"
                    problem.simulate(o1_solution_set[o1_id], int(batch_sizes[1]))
                    expended_budget += batch_sizes[1]*costs[1]
                    ...
                    problem.model.factors["target_fidelity"] = "HF"
                    problem.simulate(new_solution, int(batch_sizes[0]))
                    expended_budget += batch_sizes[0] * costs[0]
                ...
                # else if reuse design points, reuse the replications
                    ...
                    if MFMC_use == True:
                        ...
                        if CRN_crude_n-visited_pts_list[reuseid[i-1]].n_reps > 0:
                            problem.model.factors["target_fidelity"] = "HF"
                            batch_hf = CRN_crude_n-visited_pts_list[reuseid[i-1]].n_reps
                            problem.simulate(visited_pts_list[reuseid[i-1]], batch_hf)
                            expended_budget += batch_hf * costs[0]
            ...
            for i in range(2 * problem.dim + 1):
                if (i == 0):
                    ...
                    while True:
                        ...
                        problem.model.factors["target_fidelity"] = "LF"
                        problem.simulate(o1_solution_set[soln_id], batch_sizes[1])
                        expended_budget += batch_sizes[1] * costs[1]
                        sample_size += batch_sizes[1]
                        sig2 = o1_solution_set[soln_id].lf_f_var
    ...
    def solve_subproblem(self, problem, corr_k, new_x, new_solution, fval, q, grad, Hessian, delta_k, \
        ...
        if new_solution.MFMC_use == True:
            ...
            expended_budget += costs[0]*new_solution.mfmc_n_rep + costs[1]*new_solution.mfmc_o1_rep
            ...
            lf_mean_n = np.mean(candidate_o1_solution.lf_f[:candidate_solution.mfmc_n_rep])
            candidate_solution.mfmc_mean = candidate_solution.objectives_mean - c1*lf_mean_n + c1*candidate_o1_solution.lf_f_mean 
    ...
    def iterate(self, k, corr_k, delta, delta_max, problem, visited_pts_list, new_x, expended_budget, budget_limit, recommended_solns, intermediate_budgets, kappa, new_solution, o1_solution_set):
        ...
        if k == 1:
            ...
            # pilot run for high-fidelity
            problem.model.factors["target_fidelity"] = "HF"
            problem.simulate(new_solution, pilot_run)
            expended_budget += costs[0]*pilot_run
            ...
            while True:
                ...
                if solve_subproblem_approximate.fun <= costs[0]*predicted_N_k:
                    ...
                    if new_solution.n_reps > o1_solution.n_reps:
                        problem.model.factors["target_fidelity"] = "LF"
                        batch_lf = new_solution.n_reps - o1_solution.n_reps + 1
                        problem.simulate(o1_solution, batch_lf)
                        expended_budget += batch_lf * costs[1]

                    # check whetehr we can finish the sampling
                    
                    lf_mean_n = np.mean(o1_solution.lf_f[:new_solution.n_reps])
                    fn = new_solution.objectives_mean - c1*lf_mean_n  + c1*o1_solution.lf_f_mean 
                    if fn < 1 and fn > -1:
                        kappa = 1/(delta[0]**2)
                    else:
                        kappa = fn/(delta[0]**2)
                    ...
                    if new_solution.n_reps >= ceil(n0)-1:
                        problem.model.factors["target_fidelity"] = "LF"
                        problem.simulate(o1_solution, int(batch_sizes[1]))
                        expended_budget += batch_sizes[1]*costs[1]
                    ...
                    problem.model.factors["target_fidelity"] = "HF"
                    problem.simulate(new_solution, int(batch_sizes[0]))
                    expended_budget += int(batch_sizes[0]) * costs[0]
    ...
    def solve(self, problem):
        ...
        expended_budget, kappa, k, corr_k  = 0, 0, 0, 1
        new_solution, recommended_solns, intermediate_budgets = [], [], [] 
        o1_solution_set, visited_pts_list = [], []
        
        while expended_budget < budget_limit:

            # Print the result at each iteration.
            if k == 0:
                print(f"{'Iter':<6} {'new_x':<20} {'delta':<20} {'correlation parameter':<25} {'Budget Expended':<18}")
                print("=" * 100)
            delta_str = "[" + ", ".join(f"{d:.4f}" for d in delta) + "]"
            print(f"{k:<6d} {str(new_x):<20} {delta_str:<20} {corr_k:<25.4f} {expended_budget:<18.2f}")
