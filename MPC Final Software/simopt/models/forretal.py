...
class Forretal(Model):
    ...
    def replicate(self, rng_list):
        ...
        noise = noise_rng.normalvariate(0, hf_sigma2 + 0.05 * np.abs(x))
        lf_noise = noise_rng.normalvariate(0, sigma2 + 0.05 * np.abs(x))
        cor = self.factors["cor"]
        
        hfn_eval_at_x = float((6*x-2)**2*np.sin(12*x-4) + noise)
        #lfn_eval_at_x = float((1-cor**2-2*cor)*((6*x-2)**2*np.sin(12*x-4)) + 10*(x-0.5)-5 + (noise + lf_noise)/2)
        lfn_eval_at_x = float((-2+4*cor-cor**2)*((6*x-2)**2*np.sin(12*x-4)) + 10*(x-0.5)-5 + (noise + lf_noise)/2)
...
class ForretalProblem(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="FORRETAL-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = Forretal(self.model_fixed_factors)
