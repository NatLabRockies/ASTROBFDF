...
class SSContMF(Model):
    ...
    def replicate(self, rng_list):
        ...
        lf_t = self.factors["warmup"]+30
        avg_order_costs_lf = np.mean(self.factors["fixed_cost"] * (orders_placed[self.factors["warmup"]:lf_t] > 0) +
                                  self.factors["variable_cost"] * orders_placed[self.factors["warmup"]:lf_t])
        avg_holding_costs_lf = np.mean(self.factors["holding_cost"] * end_inv[self.factors["warmup"]:lf_t] * [end_inv[self.factors["warmup"]:lf_t] > 0])
        on_time_rate_lf = 1 - np.sum(np.min(np.vstack((demands[self.factors["warmup"]:lf_t], demands[self.factors["warmup"]:lf_t] - start_inv[self.factors["warmup"]:lf_t])), axis=0)
                                  * ((demands[self.factors["warmup"]:lf_t] - start_inv[self.factors["warmup"]:lf_t]) > 0))/np.sum(demands[self.factors["warmup"]:lf_t])
        avg_backorder_costs_lf = self.factors["backorder_cost"]*(1 - on_time_rate_lf)*np.sum(demands[self.factors["warmup"]:lf_t])/float(lf_t-self.factors["warmup"])
        ...
        if self.factors["s"] > self.factors["S"]:
            avg_order_costs += penalty
            avg_order_costs_lf += penalty
        ...
        responses = {"avg_backorder_costs": avg_backorder_costs,
                     ...
                     "avg_order": avg_order,
                     "avg_backorder_costs_lf": avg_backorder_costs_lf,
                     "avg_order_costs_lf": avg_order_costs_lf,
                     "avg_holding_costs_lf": avg_holding_costs_lf,
                     }
...
class SSContMinCostMF(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="SSCONTMF-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = SSContMF(self.model_fixed_factors)
    ...
    def response_dict_to_lf_f(self, response_dict):
        ...
        objectives : tuple
            vector of objectives
        """
        lf_f = (response_dict["avg_backorder_costs_lf"] + response_dict["avg_order_costs_lf"] + response_dict["avg_holding_costs_lf"],)
        return lf_f
