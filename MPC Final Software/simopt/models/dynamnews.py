...
class DynamNews(Model):
    ...
    def replicate(self, rng_list: list[MRG32k3a]) -> tuple[dict, dict]:
        ...
        # Calculate profit.
        lf_numsold = self.factors["init_level"] - lf_inventory
        lf_revenue = lf_numsold * np.array(self.factors["price"])
        cost = self.factors["init_level"] * np.array(self.factors["cost"])
        lf_profit = lf_revenue - cost
...
class DynamNewsMaxProfit(Problem):
    ...
    def __init__(self, name="DNEWS-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        if model_fixed_factors is None:
            model_fixed_factors = {}
        self.name = name
        self.dim = NUM_PRODUCTS
        self.n_objectives = 1
