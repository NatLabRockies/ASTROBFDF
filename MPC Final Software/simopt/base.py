    ...
    def simulate(self, solution: "Solution", m: int = 1):
        ...
        if m < 1:
            ...
            for _ in range(m):
                ...
                solution.lf_f[solution.n_reps] = [sum(pairs) for pairs in zip(self.response_dict_to_lf_f(responses), solution.det_objectives)]
                #if self.gradient_available:
...
class Solution(object):
    ...
    def __init__(self, x, problem):
        ...
        self.lf_f = np.zeros((init_size, problem.n_objectives))
    ...
    def pad_storage(self, m: int):
        ...
        self.lf_f = np.concatenate((self.lf_f, np.zeros((pad_size, n_objectives))))
    ...
    def recompute_summary_statistics(self):
        ...
        self.lf_f_mean = np.mean(self.lf_f[:self.n_reps], axis=0)
        if self.n_reps > 1:
            ...
            self.lf_f_var = np.var(self.lf_f[:self.n_reps], axis=0, ddof=1)
            self.lf_f_stderr = np.std(self.lf_f[:self.n_reps], axis=0, ddof=1) / np.sqrt(self.n_reps)
            self.lf_f_cov = np.cov(self.lf_f[:self.n_reps], rowvar=False, ddof=1)

            # covariance between lf and hf
            self.hf_lf_cov = (np.mean([x*y for x, y in zip(self.objectives[:self.n_reps], self.lf_f[:self.n_reps])], axis=0) - self.objectives_mean*self.lf_f_mean)
