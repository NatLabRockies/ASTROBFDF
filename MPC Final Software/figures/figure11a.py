...
#plt.plot(S_values, objective_high, label=r"$F^h(s=500,\cdot,\xi)$", linewidth=3)
...
#plt.plot(S_values, objective_low, label=r"$F^\ell(s=500,\cdot,\xi)$", linewidth=3, linestyle='--')
plt.xlabel("$S$", fontsize=20)
plt.ylabel("Objective Value", fontsize=20)
#plt.title("Objective vs. $S$ (Fixed $s=500$)", fontsize=24)
plt.legend(fontsize=16)
plt.grid(True)
plt.tight_layout()
plt.savefig("experiments/plots/figure11a.pdf", format='pdf', dpi=300)
plt.show()
