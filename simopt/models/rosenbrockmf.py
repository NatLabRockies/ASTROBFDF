...
class Rosenbrock(Model):
    ...
    def replicate(self, rng_list):
        ...
        for i in range(self.dim-1):
            ...
            noise2 = noise_rng.normalvariate(0, lf_sigma2 + 0.05 * np.abs(x[0]))
            #hfn_eval_at_x += 100*(x[i+1]-noise*x[i]**2)**2 + (noise*x[i]-1)**2
            #lfn_eval_at_x += 50*(x[i+1]-noise*x[i]**2)**2 + (-2-x[i])**2 - 0.5*x[i+1] + noise2/2
            hfn_eval_at_x += 100*(x[i+1]-x[i]**2)**2 + (x[i]-1)**2 + noise
            lfn_eval_at_x += cor*(50*(x[i+1]-x[i]**2)**2 + (-2-x[i])**2) - 0.5*x[i+1] + (noise+noise2)/2

        lfn_eval_at_x += -0.5*x[-1]
...
class RosenbrockProblem(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="ROSENBROCK-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = Rosenbrock(self.model_fixed_factors)
