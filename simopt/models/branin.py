...
class Branin(Model):
    ...
    def replicate(self, rng_list):
        ...
        lf_sigma2 = self.factors["sigma2"]
        noise = noise_rng.normalvariate(0,hf_sigma2+ 0.05 * np.abs(x[0]))
        noise_lf = noise_rng.normalvariate(0,lf_sigma2+ 0.05 * np.abs(x[0]))
        deterministic_hfn = (x[1]-(5.1*x[0]**2/(4*np.pi**2))+(5*x[0]/np.pi)-6) + 10*(1-1/(8*np.pi))*np.cos(x[0]) + 10
        hfn_eval_at_x = float(deterministic_hfn + noise)
        lfn_eval_at_x = float(deterministic_hfn - (0.5 * cor**2 -2* cor + 1.7)*(x[1] - 5.1*x[0]**2/(4*np.pi**2) + 5*x[0]/np.pi - 6)**2  + (noise + noise_lf)/2)
...
class BraninProblem(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="BRANIN-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = Branin(self.model_fixed_factors)
    ...
    def deterministic_hf_objective(self, x):
        ...
        -------
        value : float
            deterministic high-fidelity objective value
        """
        x = np.array(x)
        value = (x[1] - (5.1 * x[0] ** 2 / (4 * np.pi ** 2)) + (5 * x[0] / np.pi) - 6) \
