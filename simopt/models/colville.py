...
class Colville(Model):
    ...
    def __init__(self, fixed_factors=None):
        ...
        self.specifications = {
            "x": {
                "description": "point to evaluate from -1 to 1",
                "datatype": tuple,
                "default": (0.5, 0.5, 0.5, 0.5)
            },
            "cor": {
                "description": "correlation between HF and LF from 0 to 1",
                "datatype": float,
                "default": 1
            },
            "sigma2": {
                "description": "lower fidelity oracle s.d.",
                "datatype": float,
                "default": 50
            },
            "sigma1": {
    ...
    def replicate(self, rng_list):
        ...
        cor = self.factors["cor"]
        sigma2 = self.factors["sigma2"]
        hf_sigma2 = self.factors["sigma1"]

        #noise = noise_rng.normalvariate(0,3*hf_sigma2)
        #noise2 = noise_rng.normalvariate(0,3*sigma2)
        noise = noise_rng.normalvariate(0,hf_sigma2+ 0.05 * np.abs(x[0]))
        noise2 = noise_rng.normalvariate(0,sigma2+ 0.05 * np.abs(x[0]))
        #noise = noise_rng.normalvariate(0,1)
        #noise2 = noise_rng.normalvariate(1,10)
        
        deterministic_hfn = 100*(x[0]**2-x[1])**2 + (x[0]-1)**2 + (x[2]-1)**2 + 90*(x[2]**2-x[3]) +\
              10.1*((x[1]-1)**2+(x[3]-1)**2) + 19.8*(x[1]-1)*(x[3]-1)
        deterministic_lhfn = deterministic_hfn * (cor)**2 - (cor+0.5) * (5 * x[0]**2 + 4 * x[1]**2 + 3 * x[2]**2 + x[3]**2)
        #deterministic_lhfn = 100*(A[0]**2-A[1])**2 + (A[0]-1)**2 + (A[2]-1)**2 + 90*(A[2]**2-A[3]) +\
        #      10.1*((A[1]-1)**2+(A[3]-1)**2) + 19.8*(A[1]-1)*(A[3]-1)
        
        hfn_eval_at_x = float(deterministic_hfn + noise)
        lfn_eval_at_x = float(deterministic_lhfn) + (noise2 + noise)/2
...
class ColvilleProblem(Problem):
    ...
    base.Problem
    """
    def __init__(self, name="COLVILLEs-1", fixed_factors=None, model_fixed_factors=None):
        if fixed_factors is None:
            fixed_factors = {}
        ...
        self.model = Colville(self.model_fixed_factors)
    ...
    def deterministic_hf_objective(self, x):
        """
        Evaluate the deterministic high-fidelity objective at x.
        """
        x = np.array(x)
        value = 100 * (x[0] ** 2 - x[1]) ** 2 + (x[0] - 1) ** 2 + (x[2] - 1) ** 2 \
            + 90 * (x[2] ** 2 - x[3]) + 10.1 * ((x[1] - 1) ** 2 + (x[3] - 1) ** 2) + 19.8 * (x[1] - 1) * (x[3] - 1)
        return float(value)
