...
class Curve(object):
    ...
    def plot(self, color_str: str = "C0", curve_type: str = "regular", linestyle: Union[str, tuple, None] = None) -> list["plt.Line2D"]:
        """Plot a curve.
...
class ProblemSolver(object):
    ...
    def run(self, n_macroreps: int):
        ...
        with Pool(processes=6) as process_pool:
...
def post_normalize(experiments: list["ProblemSolver"], n_postreps_init_opt: int, crn_across_init_opt: bool = True, proxy_init_val: Union[float, None] = None, proxy_opt_val: Union[float, None] = None, proxy_opt_x: Union[tuple,  None] = None):
    ...
    deterministic_hf_objective = getattr(ref_experiment.problem, "deterministic_hf_objective", None)
    has_deterministic_hf_objective = callable(deterministic_hf_objective)
    ...
    elif has_deterministic_hf_objective:
        x0_postreps = [deterministic_hf_objective(x0)] * n_postreps_init_opt
    else:
    ...
    # ...else if x* is known...
        ...
        ref_experiment.problem.simulate(solution=opt_soln, m=n_postreps_init_opt)
        xstar_postreps = list(opt_soln.objectives[:n_postreps_init_opt][:, 0])  # 0 <- assuming only one objective
    # ...else if deterministic high-fidelity objective is available,
    # choose x* from all recommended solutions using deterministic values.
    elif has_deterministic_hf_objective:
        print("\t...using deterministic high-fidelity objective over recommended solutions as proxy for x*.")
        best_score = -np.inf
        xstar = None
        for experiment in experiments:
            for mrep in range(experiment.n_macroreps):
                for x in experiment.all_recommended_xs[mrep]:
                    score = experiment.problem.minmax[0] * deterministic_hf_objective(x)
                    if score > best_score:
                        best_score = score
                        xstar = x
        xstar_postreps = [deterministic_hf_objective(xstar)] * n_postreps_init_opt
...
def _canonical_solver_family(solver_name: str) -> str:
    upper_name = solver_name.upper()
    if "ASTRO-BFDF" in upper_name or "ASTROBFDF" in upper_name:
        return "ASTRO-BFDF"
    if "ASTRO-DF" in upper_name or "ASTRODF" in upper_name:
        return "ASTRO-DF"
    if "BFBO" in upper_name:
        return "BFBO"
    if "BFSAG" in upper_name:
        return "BFSAG"
    if "ADAM" in upper_name:
        return "ADAM"
    token = solver_name.replace("-", " ").replace("_", " ").split()[0]
    return token.upper()


def _series_linestyle(idx: int):
    style_cycle = ["-", "--", "-.", ":", (0, (3, 1, 1, 1)), (0, (5, 1))]
    return style_cycle[idx % len(style_cycle)]


def _solver_linestyle(solver_name: str):
    family_style_map = {
        "ASTRO-BFDF": "-",
        "ASTRO-DF": "--",
        "ADAM": "-.",
    ...
    family = _canonical_solver_family(solver_name)
    if family in family_style_map:
        return family_style_map[family]
    return _series_linestyle(abs(hash(family)) % 6)


def _get_solver_plot_linestyle(experiment: "ProblemSolver", idx: int, style_mode: str):
    if style_mode == "series":
        return _series_linestyle(idx)
    return _solver_linestyle(experiment.solver.name)
def plot_progress_curves(experiments: list["ProblemSolver"], plot_type: str, beta: float = 0.50, normalize: bool = True, all_in_one: bool = True, n_bootstraps: int = 100, conf_level: float = 0.95, plot_CIs: bool = True, print_max_hw: bool = True, style_mode: str = "solver") -> list[str]:
    ...
    if all_in_one:
        ...
        for exp_idx in range(n_experiments):
            ...
            if experiment.solver.name == 'BFBO with sample size=10':
                color_str = "C" + str(6)
            elif experiment.solver.name == 'BFBO with sample size=20':
                color_str = "C" + str(7)

            line_style = _get_solver_plot_linestyle(experiment, exp_idx, style_mode)
            if plot_type == "all":
                # Plot all estimated progress curves.
                if normalize:
                    line_style = _get_solver_plot_linestyle(experiment, exp_idx, style_mode)
                    handle = experiment.progress_curves[0].plot(color_str=color_str, linestyle=line_style)
                    for curve in experiment.progress_curves[1:]:
                        curve.plot(color_str=color_str, linestyle=line_style)
                else:
                    handle = experiment.objective_curves[0].plot(color_str=color_str, linestyle=line_style)
                ...
                line_style = _get_solver_plot_linestyle(experiment, exp_idx, style_mode)
                handle = estimator.plot(color_str=color_str, linestyle=line_style)
            elif plot_type == "quantile":
...
def plot_solvability_cdfs(experiments: list["ProblemSolver"], solve_tol: float = 0.1, all_in_one: bool = True, n_bootstraps: int = 100, conf_level: float = 0.95, plot_CIs: bool = True, print_max_hw: bool = True, style_mode: str = "solver") -> list[str]:
...
def plot_solvability_profiles(experiments: list[list["ProblemSolver"]], plot_type: str, all_in_one: bool = True, n_bootstraps: int = 100, conf_level: float = 0.95, plot_CIs: bool = True, print_max_hw: bool = True, solve_tol: float = 0.1, beta: float = 0.5, ref_solver: str = None, style_mode: str = "solver") -> list[str]:
    ...
    if all_in_one:
        ...
        for solver_idx in range(n_solvers):
            ...
            if plot_type in {"cdf_solvability", "quantile_solvability"}:
                line_style = _get_solver_plot_linestyle(experiments[solver_idx][0], solver_idx, style_mode)
                handle = solver_curve.plot(color_str=color_str, linestyle=line_style)
                solver_curve_handles.append(handle)
                if plot_CIs or print_max_hw:
                    # Note: "experiments" needs to be a list of list of ProblemSolver objects.
        ...
        if plot_type == "cdf_solvability":
            ...
            for solver_idx in range(n_solvers):
                if solver_idx is not ref_solver_idx:
                    ...
                    color_str = "C" + str(solver_idx)
                    line_style = _get_solver_plot_linestyle(experiments[solver_idx][0], solver_idx, style_mode)
                    handle = diff_solver_curve.plot(color_str=color_str, linestyle=line_style)
                    solver_curve_handles.append(handle)
                    if plot_CIs or print_max_hw:
                        # Note: "experiments" needs to be a list of list of ProblemSolver objects.
